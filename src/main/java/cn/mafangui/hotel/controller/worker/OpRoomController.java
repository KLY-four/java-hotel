...
public class OpRoomController {
    ...
    @RequestMapping(method = RequestMethod.POST,value = "/RoomNumber/{RoomNumber}")
    public AjaxResult getRoomInfo(@PathVariable Integer RoomNumber){
        List<Room> byRoomNumber = roomService.getByRoomNumber(RoomNumber);
        if(byRoomNumber!=null) {
            return ResponseTool.success(byRoomNumber);
        }else {return ResponseTool.failed("未查询到房间信息");}
